...
HOTBAR.forEach((item, i) => {
  ...
  slot.id = `slot-${i}`
  slot.innerHTML = `
    <div class="slot-icon" style="background:${item.color};border:1px solid rgba(255,255,255,0.2)"></div>
...
const JOY_R = 45 // max thumb travel in px
...
joyZone.addEventListener('touchmove', e => {
  ...
  for (const t of e.changedTouches) {
    if (t.identifier === joyTouch) updateJoy(t.clientX, t.clientY)
  }
}, { passive: false })
...
// Inject joystick into noa input state every tick
const DEAD = 0.25
noa.on('tick', () => {
  if (joyTouch === null) {
...
lookZone.addEventListener('touchmove', e => {
  ...
  for (const t of e.changedTouches) {
    ...
    if (noa.camera.pitch >  MAX_P) noa.camera.pitch =  MAX_P
    if (noa.camera.pitch < -MAX_P) noa.camera.pitch = -MAX_P
  }
}, { passive: false })
...
onBtn('btn-jump', () => {
  const body = noa.ents.getPhysicsBody(noa.playerEntity)
  if (body && body.onGround) body.velocity[1] = 8
  if (flying) { const b = noa.ents.getPhysicsBody(noa.playerEntity); if(b) b.velocity[1] = 8 }
})
...
// Swipe hotbar to change block
let hotbarSwipeX = 0
const hotbarEl2 = document.getElementById('hotbar')
hotbarEl2.addEventListener('touchstart', e => {
  hotbarSwipeX = e.touches[0].clientX
}, { passive: true })
hotbarEl2.addEventListener('touchend', e => {
...
window.startGame = function() {
  ...
  if (!isMobile) noa.container.setPointerLock(true)
  // Try landscape lock
