...
function _addTerritory(bx, by, bz) {
  ...
  // 预分配脉冲收缩环（从底圈全尺寸向上收缩至穹顶顶点；单一 Mesh 复用）
  var prGeo  = new THREE.TorusGeometry(R, 0.18, 6, 48);
  ...
  prMesh.position.set(cx, by + 0.3, cz);
  prMesh.visible = false;
  scene.add(prMesh);

  _territoryList.push({ cx: cx, cz: cz, r: R, baseY: by,
    mesh: meshInner, meshOut: meshWire, ring: meshRing,
    pulseRing: prMesh, pulseT: 2.0, pulseFrac: 1.0 });
}
...
function updateTerritoryFx(dt) {
  ...
  for (i = 0; i < _territoryList.length; i++) {
    ...
    // 每 2 秒触发一次：圆圈从底圈全尺寸向上收缩至穹顶顶点后消失（复用预分配 Mesh）
    t.pulseT -= dt;
    if (t.pulseT <= 0) {
      t.pulseT = 2.5;          // 2.5s 间隔（动画本身 2s，0.5s 空白）
      t.pulseFrac = 0.0;
      if (t.pulseRing) {
        t.pulseRing.visible = true;
        t.pulseRing.scale.set(1.0, 1, 1.0);
        t.pulseRing.position.y = t.baseY + 0.3;
        t.pulseRing.material.opacity = 0.7;
      }
    }

    // 每帧推进：scale XZ 按半球余弦曲线收缩，Y 轴线性上升至顶点
    if (t.pulseRing && t.pulseRing.visible) {
      t.pulseFrac += dt / 2.0;        // 2s 动画时长
      if (t.pulseFrac >= 1.0) {
        t.pulseRing.visible = false;
      } else {
        frac = t.pulseFrac;
        var sc = Math.cos(frac * Math.PI * 0.5);   // 1→0 沿穹顶曲线
        t.pulseRing.scale.set(sc, 1, sc);
        t.pulseRing.position.y = t.baseY + 0.3 + frac * t.r;
