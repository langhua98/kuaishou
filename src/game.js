...
var _placePos = null;   // 当前帧放置预览坐标（null=不可见）
...
function tick(now) {
  ...
  // 预览坐标 = 准星命中方块的前一格（selHit 已计算，直接复用）
  _placePos = (selHit && selHit.prev) ? selHit.prev : null;

  if (_placePos) {
    placeBox.position.set(_placePos.x + 0.5, _placePos.y + 0.5, _placePos.z + 0.5);
