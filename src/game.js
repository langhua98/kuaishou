...
function tick(now) {
  ...
  var _heldForPlace = player.inv[player.slot];
  if (_place.pos && !(typeof GUN !== 'undefined' && _heldForPlace === GUN)) {
  ...
  if (player.placeQ || (placeHeld && nowS - _lastPlace > PLACE_CD)) {
    ...
    if (pv) {
      ...
      if (pv.x < px0 || pv.x > px1 || pv.y < py0 || pv.y > py1 || pv.z < pz0 || pv.z > pz1) {
        ...
        if (typeof GUN !== 'undefined' && _placedId === GUN) {
          /* 持枪不放块 */
        } else if (_placedId === TOWER_ITEM) {
