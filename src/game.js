...
function tick(now) {
  ...
  if (_place.pos) {
    ...
    if (_place.lastDir && _place.lastPos) {
      ...
      // ny 下界保护：不能预测到地面以下
      if (_ny >= 0) {
        var _tx = _nx + 0.5 - camera.position.x;
        var _ty = _ny + 0.5 - camera.position.y;
        var _tz = _nz + 0.5 - camera.position.z;
        var _tlen = Math.sqrt(_tx * _tx + _ty * _ty + _tz * _tz);
        // FOV 门控：点积 > cos(45°) ≈ 0.707，且距离 ≤ 8 格
        if (_tlen > 0 &&
