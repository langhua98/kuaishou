...
// ── 放置预览框（placeBox）+ 方向学习放置系统 ──────────────────────────────────
// 放置预览框：半透明绿色虚线，套在「将要放置的空格」上，随视角实时移动；
// 方向学习：每次放置后记录 lastPos + 从 lastPos→当前 学到的主轴方向 lastDir，
//   下一帧预览框自动锁到 lastPos+lastDir 处；转弯时立刻学到新方向，无需重置。
...
function tick(now) {
  ...
  // 计算本帧预览坐标
  // selHit 已在上方计算，直接复用，避免第二次 raycast（Fix F1）
  _place.pos = null;
  if (selHit && selHit.prev) _place.pos = selHit.prev;

  if (!_place.pos && _place.lastDir && _place.lastPos) {
    // 兜底：准星朝天/空旷时，沿 lastDir 从 lastPos 延伸预测下一格
    var nx = _place.lastPos.x + _place.lastDir.x;
    var ny = _place.lastPos.y + _place.lastDir.y;
    var nz = _place.lastPos.z + _place.lastDir.z;
    var tries = 0;
    while (tries < 8 && ny >= 0) {               // ny>=0 防止向下钻入 y<0（Fix C2）
      var blk = getBlock(nx, ny, nz);             // 单次调用，不重复查（Fix D3）
      if (blk === AIR || blk === WATER) break;
      nx += _place.lastDir.x; ny += _place.lastDir.y; nz += _place.lastDir.z;
      tries++;
    }
    if (tries < 8 && ny >= 0) {
  ...
  if (player.placeQ || (placeHeld && nowS - _lastPlace > PLACE_CD)) {
    ...
    var pv = _place.pos;
    if (pv) {
      // 碰撞体全范围防重叠：覆盖玩家 AABB 实际占据的所有体素（Fix C4）
