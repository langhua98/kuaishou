  ...
  for (var s = 0; s < stations.length; s++) {
    ...
    (function(stationZ) {
      gltfLoader.load('assets/models/train/train_station.glb', function(g) {
        ...
        // PBR→Lambert（与游戏风格一致，避免 PBR 在低光照下变黑）
        m.traverse(function(node) {
          if (!node.isMesh || !node.material) return;
          var src = Array.isArray(node.material) ? node.material[0] : node.material;
          var hex = (src && src.color) ? src.color.getHex() : 0xcccccc;
          node.material = new THREE.MeshLambertMaterial({ color: hex });
        });
        scene.add(m);
      }, undefined, function(err) { console.error('train_station load err', err); });
    })(sz);
  }
}
