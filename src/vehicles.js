...
_VEHICLE_DEFS[VEH_BMW]    = { name:'🏎️宝马330i', gltf:'bmw_330i_v2.glb', speed:24, turnSpd:2.1, color:0x9aa3ad, w:1.0, h:0.55, d:2.1, modelLen:4.6, env:true };
...
function _applyVehicleEnv(model) {
  if (!_vehEnvTex && typeof renderer !== 'undefined' && renderer && THREE.PMREMGenerator) {
    ...
    es.background = new THREE.Color(0xe8e8e8);   // 亮一点的中性环境，黑色车漆才反出高光不发黑
  ...
  model.traverse(function (node) {
    ...
    for (var mi = 0; mi < mats.length; mi++) {
      var m = mats[mi];
      if (!m) continue;
      if (_vehEnvTex) { m.envMap = _vehEnvTex; m.envMapIntensity = 1.6; }
      m.needsUpdate = true;
    }
  });
}
