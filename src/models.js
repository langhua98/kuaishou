...
// 裁剪平面：material.clippingPlanes 在【世界空间】定义，渲染器再乘 camera.matrixWorldInverse
// 转到视图空间裁剪。手臂挂在 camera 下（世界 y≈玩家身高 60+），固定世界平面会整条裁掉。
// 故保留「视图空间目标平面」_fpClip*V，每帧乘 camera.matrixWorld 写入 _fpClip*（世界平面），
// 渲染器再乘 matrixWorldInverse 抵消 → 实际在视图空间裁剪，跟随相机。
var _fpClipTopV = null, _fpClipBotV = null;   // 视图空间目标
var _fpClipTop  = null, _fpClipBot  = null;   // 世界空间（材质引用，每帧更新）
...
function loadPlayerModel() {
  gltfLoader.load('assets/models/player.glb', function (gltf) {
    ...
    // ── 第一人称 GLB 手臂 ─────────────────────────────────────────────────────
    // clone gltf.scene → 重绑骨架至 clone 内节点 → 挂到 camera 空间
    _fpArmScene = gltf.scene.clone(true);

    // 骨架重绑定：clone 后 SkinnedMesh.skeleton 仍指向原始骨骼，须重定向到 clone 内同名节点
    var _fpBoneMap = {};
    ...
    // 裁剪平面在【视图空间】定义（每帧由 _updateFpClip 转到世界空间）：
    //   当前布局 scale=0.4, pos_y=-0.52 时（视图空间 = camera 局部空间）：
    //     肩高(1.3m) → view_y = -0.52 + 1.3×0.4 = 0
    //     手部(0.7m) → view_y = -0.52 + 0.7×0.4 = -0.24
    //     脚踝(0.1m) → view_y = -0.52 + 0.1×0.4 = -0.48
    //   上平面 clip view_y > 0.05：截去头/上肩（已由 Soldier_head hide 补充）
    //   下平面 clip view_y < -0.35：截去小腿/脚踝
    //   可见区间 view_y ∈ [-0.35, 0.05] ≈ 前臂到上臂（含躯干侧面）
    // · frustumCulled=false — 相机空间 SkinnedMesh 包围盒在世界空间被错误剔除导致不可见
    renderer.localClippingEnabled = true;
    _fpClipTopV = new THREE.Plane(new THREE.Vector3(0, -1, 0),  0.05);
    _fpClipBotV = new THREE.Plane(new THREE.Vector3(0,  1, 0),  0.35);
    _fpClipTop  = new THREE.Plane();
    _fpClipBot  = new THREE.Plane();
    _fpArmScene.traverse(function (n) {
    ...
    // 独立 Mixer（直接复用已被 _lockRootMotion 处理过的 gltf.animations 片段）
    ...
    // 延迟加载兼容：若 GLB 加载完成时已处于第一人称模式，立即切换
...
// 每帧把视图空间目标裁剪平面转成世界空间（material.clippingPlanes 要求世界空间）。
// 渲染器内部会再乘 camera.matrixWorldInverse，与此处的 matrixWorld 抵消，
// 净效果 = 裁剪平面固定在视图空间，跟随相机移动/旋转，手臂始终被正确截取。
// 须在 renderer.render 之前调用，且 camera.matrixWorld 已是当前帧值。
function _updateFpClip() {
  if (!_fpClipTop || !_fpArmScene || !_fpArmScene.visible) return;
  camera.updateMatrixWorld();
  _fpClipTop.copy(_fpClipTopV).applyMatrix4(camera.matrixWorld);
  _fpClipBot.copy(_fpClipBotV).applyMatrix4(camera.matrixWorld);
}
