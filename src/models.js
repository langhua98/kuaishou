...
// LoopOnce 动画进行中锁（防止运动状态机在同帧打断一次性动画）
var _animOnceLock = false;
...
function loadPlayerModel() {
  gltfLoader.load('assets/models/player.glb', function (gltf) {
    ...
    // Three.js 可能修改骨骼名（冒号/大小写），用宽松匹配：含 RightHand 且不含手指关键词
    _rhBone3P = null;
    var _rhExclude = /Index|Ring|Middle|Pinky|Thumb/i;
    model.traverse(function (n) {
      if (!_rhBone3P && n.name && n.name.indexOf('RightHand') !== -1 && !_rhExclude.test(n.name)) {
        _rhBone3P = n;
      }
    });
    // 兜底：从 SkinnedMesh.skeleton.bones 搜索（某些 GLB 骨骼不挂到场景图）
    if (!_rhBone3P) {
      model.traverse(function (n) {
        if (!n.isSkinnedMesh || !n.skeleton) return;
        var bones = n.skeleton.bones;
        for (var bi = 0; bi < bones.length; bi++) {
          var bn = bones[bi].name;
          if (bn && bn.indexOf('RightHand') !== -1 && !_rhExclude.test(bn)) {
            _rhBone3P = bones[bi]; break;
          }
        }
      });
    }
...
function loadPlayerArms() {
  gltfLoader.load('assets/models/player_arms.glb', function (gltf) {
    ...
    model.scale.set(s, s, s);
    // 把模型下移，使手部（y≈0.9 模型空间）对齐到组原点；
    // 这样 rotation.x=0.8 旋转后，手臂从屏幕底部向屏幕内延伸（吃鸡风格）
    model.position.y = -s * 0.9;
    ...
    // CF/吃鸡风格定位：
    // · rotation.x = 1.3（约74°）：模型 -Y（手臂下垂）映射到 camera -Z（入屏96%）+ 向下27%；
    //   透视投影使前臂从屏幕边缘向准心方向收敛，形成 CF 第一人称视角手臂。
    //   注：rotation.x = π/2（90°）会使手臂轴线完全垂直于屏幕，前臂变成一个点，不可取。
    // · scale = 0.30：缩小使左右手臂向屏幕中心靠拢（避免手臂贴近屏幕边缘）
    // · position = (0, -0.15, -0.22)：前臂出现在屏幕下方，往摄像头方向收回避免伸太远
    var fpScale = 0.30;
...
function playerAnim(state) {
  ...
  if (_ANIM_ONCE[state]) {
    _animOnceLock = true;   // 锁住，防止同帧运动代码打断
...
function _registerSitTransition() {
  ...
  playerMixer.addEventListener('finished', function (ev) {
    _animOnceLock = false;   // 解锁，允许后续状态切换
...
function loadGunModel() {
  gltfLoader.load('assets/models/gun.glb', function (gltf) {
    ...
    // 枪口在 Sketchfab 本地 -Y 端 → 世界 +Z（朝向玩家），绕 Y 转 PI 使枪口朝前（-Z）
...
function updateGunTransform() {
  ...
  _gunNode3P.rotation.set(0.28, player.yaw + Math.PI, 0.22);
}
