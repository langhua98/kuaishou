...
function updateGunTransform() {
  ...
  if (_rhBone3P) {
    ...
    // 骨骼未就绪兜底：玩家右侧 + 肩部高度
    var _ryx = Math.cos(player.yaw) * 0.35;
    var _ryz = -Math.sin(player.yaw) * 0.35;
    _gunNode3P.position.set(player.x + _ryx, player.y + 1.4, player.z + _ryz);
  }
  // 朝向基于玩家 yaw，枪管已指向 -Z 方向，旋转后朝向角色前方
